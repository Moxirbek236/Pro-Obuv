...
MEDIA_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'static', 'media', 'news')
...
def save_news_data(data):
    ...
    try:
        ...
        data["metadata"]["active_count"] = len([n for n in data["news"] if n.get("is_active", False)])
...
def get_all_news():
    ...
    try:
        ...
        news_list.sort(key=lambda x: (x.get("display_order", 999), x.get("created_at", "")))
...
def update_news(news_id):
    ...
    try:
        ...
        news_item.update({
            ...
            "is_active": news_data.get('is_active', news_item.get('is_active', True)),
            "display_order": news_data.get('display_order', news_item.get('display_order', 0)),
...
def upload_media():
    ...
    try:
        ...
        return jsonify({
            "success": False,
            "message": f"Fayl yuklashda xatolik: {str(e)}"
        }), 500
